...
export async function runAgent(message: string, requestedConversationId?: string): Promise<AgentResponse> {
  ...
  if (/whats\s?app|asesor|persona|humano|contactar|contacto/.test(normalized)) {
    const url = getWhatsAppUrl("Hola, vengo de Glocal Agent y quiero recibir información.");
    responseMessage = "Claro. Te llevo con el equipo de Glocal Live por WhatsApp para que continúes la conversación.";
    if (url) ui = [{ type: "contact", channel: "whatsapp", title: "Habla con Glocal Live", description: "Abre WhatsApp y cuéntanos qué evento buscas o qué experiencia quieres organizar.", url, cta: "Continuar en WhatsApp" }];
  } else if (/organizar|organizador|crear (?:mi |un )?evento|publicar (?:mi |un )?evento|promotor|producir (?:mi |un )?evento|cotizar/.test(normalized)) {
    const url = getWhatsAppUrl("Hola, quiero organizar o publicar un evento con Glocal Live.");
    responseMessage = "Podemos ayudarte a publicar tu evento, definir accesos y conectar la operación con la boletera. Para revisar tu proyecto, continúa con nuestro equipo por WhatsApp.";
    if (url) ui = [{ type: "contact", channel: "whatsapp", title: "Organiza con Glocal Live", description: "Comparte ciudad, fecha estimada, tipo de evento y aforo esperado para orientarte mejor.", url, cta: "Cotizar mi evento" }];
  } else if (/evento|este fin|fin de semana|qué tienen|que tienen|salir/.test(normalized) && !/boleto|ticket|vip|general/.test(normalized)) {
